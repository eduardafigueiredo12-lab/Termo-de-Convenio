...
const WORD_LEGACY_HIGH_ORDER_WORDS = [
  [0xE1, 0xF0],
  [0x1D, 0x0F],
  [0xCC, 0x9C],
  [0x84, 0xC0],
...
const WORD_LEGACY_ENCRYPTION_MATRIX = [
...
function senhaWordLegadoParaBytes(senha) {
  const passwordBytes = Buffer.from(String(senha), "utf8").subarray(0, 15);
  const passwordLength = passwordBytes.length;
  const highOrderWord = passwordLength > 0
    ? [...WORD_LEGACY_HIGH_ORDER_WORDS[passwordLength - 1]]
    : [0x00, 0x00];

  for (let i = 0; i < passwordLength; i++) {
    const passwordByte = passwordBytes[i];
    const matrixIndex = i + WORD_LEGACY_ENCRYPTION_MATRIX.length - passwordLength;
    for (let bit = 0; bit < WORD_LEGACY_ENCRYPTION_MATRIX[0].length; bit++) {
      if ((passwordByte & (1 << bit)) === 0) continue;
      highOrderWord[0] ^= WORD_LEGACY_ENCRYPTION_MATRIX[matrixIndex][bit][0];
      highOrderWord[1] ^= WORD_LEGACY_ENCRYPTION_MATRIX[matrixIndex][bit][1];
    }
  }

  let lowOrderWord = 0x0000;
  for (let i = passwordLength - 1; i >= 0; i--) {
    lowOrderWord = ((((lowOrderWord >>> 14) & 0x0001) | ((lowOrderWord << 1) & 0x7fff)) ^ passwordBytes[i]) & 0xffff;
  }
  lowOrderWord = ((((lowOrderWord >>> 14) & 0x0001) | ((lowOrderWord << 1) & 0x7fff)) ^ passwordLength ^ 0xce4b) & 0xffff;
  ...
  return Buffer.from(key.toString("hex").toUpperCase(), "utf16le");
}

function gerarHashProtecaoWord(senha, salt, spinCount = WORD_PROTECTION_SPIN_COUNT) {
  let hash = crypto
    .createHash("sha512")
    .update(Buffer.concat([salt, senhaWordLegadoParaBytes(senha)]))
    .digest();
